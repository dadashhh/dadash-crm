...
// =============================================
// PROVIDER DASHBOARD TAB — Provider role, Page 1
// =============================================
const ProviderDashboardTab = ({user, lang, txs, providerPayouts: ppData, profiles, models}) => {
  const { convertAmount, fmtAmount, currencySymbol } = useCurrency();
  const fr = lang === "fr";

  const providerTxs = useMemo(() => txs.filter(tx => tx.provider_id === user.id), [txs, user.id]);
  const validTxs = useMemo(() => providerTxs.filter(tx => tx.status === "validated" || tx.status === "confirmee"), [providerTxs]);
  const pendingTxs = useMemo(() => providerTxs.filter(tx => tx.status === "pending"), [providerTxs]);
  const totalCA = validTxs.reduce((s, tx) => s + convertAmount(Number(tx.amount), tx.currency), 0);
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  const thisMonthCA = validTxs.filter(tx => new Date(tx.date) >= monthStart).reduce((s, tx) => s + convertAmount(Number(tx.amount), tx.currency), 0);

  const myPayouts = (ppData || []).filter(p => p.provider_id === user.id || p.declared_by === user.id);
  const totalPaid = myPayouts.filter(p => p.status === "validated").reduce((s, p) => s + Number(p.amount), 0);
  const totalPendingPayout = myPayouts.filter(p => p.status === "pending").reduce((s, p) => s + Number(p.amount), 0);
  ...
  const weeklyData = useMemo(() => {
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      wStart.setDate(today.getDate() - (today.getDay() || 7) - i * 7 + 1);
      const wEnd = new Date(wStart);
      wEnd.setDate(wStart.getDate() + 6);
      const wTxs = validTxs.filter(tx => {
        const d = new Date(tx.date);
        return d >= wStart && d <= wEnd;
      });
      const ca = wTxs.reduce((s, tx) => s + convertAmount(Number(tx.amount), tx.currency), 0);
      const label = `${wStart.getDate()}/${wStart.getMonth() + 1}`;
      weeks.push({ name: label, ca: Math.round(ca), txs: wTxs.length });
    }
    return weeks;
  }, [validTxs, convertAmount]);
  ...
  const topModels = useMemo(() => {
    ...
    validTxs.forEach(tx => {
      ...
      if (!mid) return;
      if (!modelMap[mid]) modelMap[mid] = { ca: 0, count: 0, name: models.find(m => m.id === mid)?.name || "?" };
      modelMap[mid].ca += convertAmount(Number(tx.amount), tx.currency);
      modelMap[mid].count++;
    });
    return Object.values(modelMap).sort((a, b) => b.ca - a.ca).slice(0, 5);
  }, [validTxs, models, convertAmount]);

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 22, fontWeight: 800, margin: 0 }}>{fr ? "Bonjour" : "Hello"} {user.name} {"\u{1F44B}"}</h2>
        <p style={{ fontSize: 12, color: "var(--text-tertiary)", margin: "4px 0 0", fontStyle: "italic" }}>{fr ? "Votre espace prestataire — vue d'ensemble" : "Your provider space — overview"}</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 28 }}>
        {[
          ...
          <div key={k.label} style={{ background: "var(--card-bg)", border: "1px solid var(--border-subtle)", borderRadius: 18, padding: "16px 18px" }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 800, color: k.color || "var(--text-primary)" }}>{k.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div className="section-title"><div className="section-bar"></div>{fr ? "CA hebdomadaire" : "Weekly revenue"}</div>
        <div style={{ height: 260 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weeklyData}>
              ...
              <RechartsTooltip contentStyle={{ background: "var(--bg-raised)", border: "1px solid var(--border-default)", borderRadius: 12, fontSize: 12 }} />
              <Bar dataKey="ca" name={fr ? "CA" : "Revenue"} fill="var(--accent)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {topModels.length > 0 && (
        <div className="card" style={{ marginBottom: 24 }}>
          <div className="section-title"><div className="section-bar"></div>{fr ? "Top modèles" : "Top models"}</div>
          {topModels.map((m, i) => {
            const maxCA = topModels[0]?.ca || 1;
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid var(--border-subtle)" }}>
                <span style={{ width: 24, textAlign: "center", fontSize: 14, fontWeight: 800, color: i === 0 ? "var(--accent)" : "var(--text-tertiary)" }}>{i + 1}</span>
                <span style={{ flex: 1, fontSize: 13, fontWeight: 600 }}>{m.name}</span>
                <span style={{ fontSize: 13, fontWeight: 700 }}>{Math.round(m.ca).toLocaleString("fr-FR")}{currencySymbol}</span>
                <span style={{ fontSize: 11, color: "var(--text-tertiary)", width: 50, textAlign: "right" }}>{m.count} TX</span>
                <div style={{ width: 100, height: 4, borderRadius: 2, background: "var(--bg-overlay)" }}>
                  <div style={{ width: (m.ca / maxCA * 100) + "%", height: "100%", borderRadius: 2, background: "var(--accent)" }} />
                </div>
              </div>
            );
          })}
      ...
      <div className="card">
        <div className="section-title"><div className="section-bar"></div>{fr ? "Dernières transactions" : "Recent transactions"}</div>
        <div className="table-wrap">
          <table className="table" style={{ fontSize: 12 }}>
            <thead><tr><th>{t(lang, "date")}</th><th>Spender</th><th>Model</th><th>{t(lang, "amount")}</th><th>{t(lang, "status")}</th></tr></thead>
            <tbody>
              {providerTxs.sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 10).map(tx => (
                <tr key={tx.id}>
              ...
              {providerTxs.length === 0 && <tr><td colSpan={5} style={{ textAlign: "center", color: "var(--text-quaternary)", padding: 30 }}>{fr ? "Aucune transaction" : "No transactions"}</td></tr>}
            </tbody>
          </table>
        </div>
        {providerTxs.length > 10 && <div style={{ fontSize: 11, color: "var(--text-tertiary)", textAlign: "center", padding: "8px 0" }}>+{providerTxs.length - 10} {fr ? "autres" : "more"}</div>}
      </div>
    </div>
  );
};
...
// =============================================
// PROVIDER TRANSACTIONS TAB — Provider role, Page 2
// =============================================
const ProviderTransactionsTab = ({user, lang, txs, models, profiles}) => {
  const { convertAmount, fmtAmount, currencySymbol } = useCurrency();
  const fr = lang === "fr";
  ...
  const filtered = useMemo(() => {
    let list = providerTxs;
    if (statusFilter !== "all") list = list.filter(tx => tx.status === statusFilter);
    if (modelFilter) list = list.filter(tx => tx.model_id === modelFilter);
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(tx =>
        (tx.spender_handle || "").toLowerCase().includes(q) ||
        (models.find(m => m.id === tx.model_id)?.name || "").toLowerCase().includes(q) ||
        String(tx.amount).includes(q)
      );
    }
    return list.sort((a, b) => new Date(b.date) - new Date(a.date));
  }, [providerTxs, statusFilter, modelFilter, search, models]);

  useEffect(() => { setPage(0); }, [statusFilter, modelFilter, search]);

  // Stats
  const validTxs = providerTxs.filter(tx => tx.status === "validated" || tx.status === "confirmee");
  const totalCA = validTxs.reduce((s, tx) => s + convertAmount(Number(tx.amount), tx.currency), 0);
  const pendingCount = providerTxs.filter(tx => tx.status === "pending").length;

  // Unique models for filter
  const modelIds = [...new Set(providerTxs.map(tx => tx.model_id).filter(Boolean))];

  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>{"\u{1F4B8}"} {fr ? "Mes Transactions" : "My Transactions"}</h2>
        <p style={{ fontSize: 11, color: "var(--text-tertiary)", margin: "3px 0 0", fontStyle: "italic" }}>{fr ? "Toutes vos transactions en détail" : "All your transactions in detail"}</p>
      </div>

      {/* Quick stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
        {[
          { label: "Total TX", value: providerTxs.length, icon: "\u{1F4CB}" },
          { label: fr ? "CA Validé" : "Validated Rev.", value: totalCA.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) + currencySymbol, icon: "\u{1F4B0}", color: "var(--success)" },
          { label: fr ? "En attente" : "Pending", value: pendingCount, icon: "\u23F3", color: pendingCount > 0 ? "var(--warning)" : undefined },
          { label: fr ? "Résultats" : "Results", value: filtered.length, icon: "\u{1F50D}" },
        ].map((k) => (
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        <input className="form-input" style={{ width: 240, fontSize: 12 }} placeholder={fr ? "Rechercher spender, model, montant..." : "Search spender, model, amount..."} value={search} onChange={e => setSearch(e.target.value)} />
        <select className="filter-select" value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ fontSize: 12 }}>
          ...
          <option value="validated">{fr ? "Validée" : "Validated"}</option>
          <option value="confirmee">{fr ? "Confirmée" : "Confirmed"}</option>
          <option value="refused">{fr ? "Refusée" : "Refused"}</option>
          <option value="refusee">{fr ? "Refusée" : "Refused"}</option>
        </select>
        <select className="filter-select" value={modelFilter} onChange={e => setModelFilter(e.target.value)} style={{ fontSize: 12 }}>
          <option value="">{fr ? "Tous modèles" : "All models"}</option>
          {modelIds.map(mid => (
            <option key={mid} value={mid}>{models.find(m => m.id === mid)?.name || mid}</option>
          ))}
        </select>
        {(search || statusFilter !== "all" || modelFilter) && (
          <button className="btn" style={{ fontSize: 11, padding: "5px 12px" }} onClick={() => { setSearch(""); setStatusFilter("all"); setModelFilter(""); }}>{fr ? "Réinitialiser" : "Reset"}</button>
        )}
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          <table className="table" style={{ fontSize: 12 }}>
            <thead><tr>
              <SortTh label={t(lang, "date")} sortKey="date" sort={sort} onSort={setSort} />
              <th>Spender</th>
              <th>Model</th>
              <SortTh label={t(lang, "amount")} sortKey="amount" sort={sort} onSort={setSort} />
              <th>{fr ? "Produit" : "Product"}</th>
              <th>Tag</th>
              <SortTh label={t(lang, "status")} sortKey="status" sort={sort} onSort={setSort} />
            </tr></thead>
            <tbody>
              {paginate(filtered, page).map(tx => (
                <tr key={tx.id}>
                  <td>{fmtDate(tx.date)}</td>
                  <td><Tag type="spender" label={tx.spender_handle} id={tx.spender_handle} /></td>
                  <td><Tag type="model" label={models.find(m => m.id === tx.model_id)?.name || "?"} id={tx.model_id} /></td>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text-quaternary)", padding: 30 }}>{fr ? "Aucune transaction trouvée" : "No transactions found"}</td></tr>}
            </tbody>
          </table>
        </div>
        <Pagination currentPage={page} setPage={setPage} totalItems={filtered.length} lang={lang} />
      </div>
    </div>
  );
};
...
// =============================================
// PROVIDER REVERSEMENTS TAB — Provider role, Page 3
// =============================================
const ProviderReversementsTab = ({user, lang, txs, providerPayouts: ppData, profiles, models, onRefresh, onNotify, invoices}) => {
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>{"\u{1F4B5}"} {fr ? "Reversements" : "Payouts"}</h2>
        <p style={{ fontSize: 11, color: "var(--text-tertiary)", margin: "3px 0 0", fontStyle: "italic" }}>{fr ? "Déclarez et suivez vos paiements" : "Declare and track your payments"}</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        {[{ id: "paiements", label: fr ? "Mes Paiements" : "My Payments" }, { id: "factures", label: fr ? "Mes Factures" : "My Invoices" }].map(tb => (
          <button key={tb.id} className="btn" style={{ padding: "6px 16px", fontSize: 12, fontWeight: subTab === tb.id ? 800 : 600, background: subTab === tb.id ? "var(--accent)" : "var(--card-bg)", color: subTab === tb.id ? "#fff" : "var(--text-secondary)", border: "1px solid " + (subTab === tb.id ? "var(--accent)" : "var(--border-subtle)"), borderRadius: 20 }} onClick={() => setSubTab(tb.id)}>{tb.label}</button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
            {[
              { label: fr ? "CA Total Traité" : "Total Revenue", value: totalCA.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) + currencySymbol, icon: "\u{1F4B0}", color: "var(--success)" },
              { label: fr ? "Paiements validés" : "Validated payments", value: totalPaid.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) + currencySymbol, icon: "\u2705", color: "var(--success)" },
              { label: fr ? "En attente" : "Pending", value: totalPendingPayout.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) + currencySymbol, icon: "\u23F3", color: "var(--warning)" },
              { label: fr ? "Solde restant" : "Remaining balance", value: balance.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) + currencySymbol, icon: balance > 0 ? "\u26A0\uFE0F" : "\u2705", color: balance > 0 ? "var(--warning)" : "var(--success)" },
            ].map((k) => (
              <div key={k.label} style={{ background: "var(--card-bg)", border: "1px solid var(--border-subtle)", borderRadius: 18, padding: "14px 16px" }}>
                <div style={{ fontSize: 10, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>{k.icon} {k.label}</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: k.color || "var(--text-primary)" }}>{k.value}</div>
...
module.exports = { PrestatairesTab, ProviderDashboardTab, ProviderTransactionsTab, ProviderReversementsTab };
