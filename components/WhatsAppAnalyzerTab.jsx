            ...
            <div className="card">
              ...
              <div style={{maxHeight: 200, overflowY: "auto", padding: 12, borderRadius: 10, background: "var(--bg-overlay)", border: "1px solid var(--border-subtle)", fontFamily: "monospace", fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.6}}>
                {messages.slice(-30).map((m, i) => (
                  <div key={"msg-" + i + "-" + m.sender}><span style={{color: "var(--text-quaternary)"}}>{m.date instanceof Date ? m.date.toLocaleString("fr-FR",{hour:"2-digit",minute:"2-digit"}) : ""}</span> <span style={{color: "var(--accent)", fontWeight: 600}}>{m.sender}:</span> {m.body.slice(0, 100)}</div>
                ))}
              </div>
            </div>
          )}
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={participantChartData} dataKey="messages" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={45}
                        ...
                        {participantChartData.map((entry, i) => <Cell key={entry.name || ("pie-cell-" + i)} fill={COLORS[i % COLORS.length]} />)}
                      </Pie>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={participantChartData} layout="vertical" margin={{left: 80}}>
                    ...
                    <Bar dataKey="messages" radius={[0, 4, 4, 0]}>
                      {participantChartData.map((entry, i) => <Cell key={entry.name || ("bar-cell-" + i)} fill={COLORS[i % COLORS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
...
module.exports.default = WhatsAppAnalyzerTab;
