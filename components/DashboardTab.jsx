...
// DASHBOARD TAB
// =============================================
const DashboardTab = React.memo(({user, lang, txs, models, profiles, onRefresh, onNotify, alerts, botStats, onDismissAlert, onAlertNavigate}) => {
  ...
  const [processingDashTxId, setProcessingDashTxId] = useState(null);
  ...
  const handleDashStatus = async (txId, newStatus) => {
    ...
    } catch(e) { addToast("Erreur: " + e.message, "error"); } finally { setProcessingDashTxId(null); }
  };
  ...
  const filtered = useMemo(() => filterByStatus(filterByChatter(filterByProvider(filterByModel(dr.filterByDate(txs, "date"), model), provider), chatter), status), [txs, dr.startDate, dr.endDate, dr.activePreset, model, provider, chatter, status, convertAmount]);
  const dashKPIs = useMemo(() => {
    const pending = filtered.filter(tx=>tx.status==="pending");
    const validated = filtered.filter(tx=>tx.status==="validated");
    return {
      pending, validated,
      caGross: filtered.reduce((s,tx)=>s+convertAmount(Number(tx.amount),tx.currency),0),
      commTotal: filtered.reduce((s,tx)=>s+convertAmount(Number(tx.chatter_commission||0),tx.currency),0),
      caNet: filtered.reduce((s,tx)=>s+convertAmount(Number(tx.net_amount||0),tx.currency),0),
      caPending: pending.reduce((s,tx)=>s+convertAmount(Number(tx.amount),tx.currency),0),
      caValidated: validated.reduce((s,tx)=>s+convertAmount(Number(tx.amount),tx.currency),0),
    };
  }, [filtered, convertAmount]);
  const { pending, validated, caGross, commTotal, caNet, caPending, caValidated } = dashKPIs;
  ...
  const { totalBrut, totalValide, totalPending: totalPendingChart } = useMemo(() => ({
  ...
  const totalPending = totalPendingChart;
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 2 }}>
              CA brut {t(lang,"period")} · {totalValide.toLocaleString("fr-FR", {maximumFractionDigits:0})}{currencySymbol} {t(lang,"validated").toLowerCase()} · {totalPending.toLocaleString("fr-FR", {maximumFractionDigits:0})}{currencySymbol} pending
            </div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
          {[
            ...
            { key: "valide", label: t(lang,"validated"), color: "var(--success)" },
            { key: "pending", label: "Pending", color: "var(--warning)" },
            { key: "refuse", label: t(lang,"refused"), color: "var(--danger)" },
          ].map(m => (
            <button
              key={m.key}
            ...
            <AreaChart data={chartData}>
              ...
              {activeMetrics.includes("valide") && (
                <Area type="monotone" dataKey="valide" stroke="var(--success)" strokeWidth={2} fill="url(#gradValide)"
                  ...
                  name={t(lang,"validated")} animationDuration={600} />
              )}
              ...
              {activeMetrics.includes("refuse") && (
                <Area type="monotone" dataKey="refuse" stroke="var(--danger)" strokeWidth={2} fill="url(#gradRefuse)"
                  ...
                  name={t(lang,"refused")} animationDuration={600} />
              )}
            </AreaChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="grid-2">
        ...
        {user.role==="gerant"?
          <div className="card">
            ...
            <div className="table-wrap">
              {profiles.filter(u=>u.role==="chatter").length === 0 ? <EmptyState icon="💬" text="Aucun revenu"/> : <table className="table"><thead><tr><th>Chatter</th><th>CA</th></tr></thead>
              <tbody>
          ...
          <div className="card">
            ...
            <div className="table-wrap">
              {filtered.length === 0 ? <EmptyState icon="💸" text="Aucune transaction"/> : <table className="table" style={{fontSize:11}}><thead><tr><th>{t(lang,"date")}</th><th>Spender</th><th>{t(lang,"amount")}</th><th>{t(lang,"product")}</th><th>Tag</th><th>{t(lang,"model")}</th><th>Chatter</th><th>Provider</th><th>{t(lang,"status")}</th></tr></thead>
              <tbody>
      ...
      <div className="card">
        <div className="section-title"><div className="section-bar"></div>{t(lang,"recent_tx")}</div>
        {filtered.length === 0 ? <EmptyState icon="💸" text="Aucune transaction"/> :
        <div className="table-wrap">
          <table className="table">
            <thead>
              <tr><th>{t(lang,"date")}</th><th>Spender</th><th>{t(lang,"amount")}</th><th>{t(lang,"product")}</th><th>Tag</th><th>{t(lang,"model")}</th><th>Chatter</th><th>Provider</th><th>{t(lang,"fees")}</th><th>{t(lang,"commission")}</th><th>{t(lang,"status")}</th><th>{t(lang,"actions")}</th></tr>
            </thead>
            <tbody>
                  ...
                  <tr key={tx.id} className={tx.status==="pending"?"pending-row":""}>
                    ...
                    <td style={{whiteSpace:"nowrap"}}>
                      ...
                      {tx.status==="pending"&&(user.role==="gerant"||(user.role==="provider"&&tx.provider_id===user.id))&&(
                        <><button className="btn btn-success btn-small" onClick={()=>handleDashStatus(tx.id,"validated")} disabled={!!processingDashTxId}>{processingDashTxId===tx.id?<><BtnSpinner/>{" "}</>:t(lang,"validate")}</button>{" "}<button className="btn btn-danger btn-small" onClick={()=>setConfirmRefuse(tx.id)} disabled={!!processingDashTxId}>{t(lang,"refuse")}</button></>
                      )}
                    </td>
                  </tr>
                  );
      ...
      <ConfirmDialog open={!!confirmRefuse} icon={"\u26A0\uFE0F"} title={lang==="fr"?"Refuser cette transaction ?":"Refuse this transaction?"} message={lang==="fr"?"Cette action est irréversible. La transaction sera marquée comme refusée.":"This action is irreversible. The transaction will be marked as refused."} confirmLabel={lang==="fr"?"Refuser":"Refuse"} cancelLabel={lang==="fr"?"Annuler":"Cancel"} danger onConfirm={()=>{handleDashStatus(confirmRefuse,"refused");setConfirmRefuse(null);}} onCancel={()=>setConfirmRefuse(null)}/>
    </div>
  );
});

module.exports.default = DashboardTab;
