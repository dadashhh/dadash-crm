// =============================================
// TELEGRAM TAB (v3 — robust pipeline)
// =============================================
const TelegramTab = React.memo(({user, lang, txs, models, profiles, spenders, onRefresh, navigateToSpender}) => {
  ...
  const tgSort = useSortable("ltv","desc");
  ...
  const tgFilteredLen = filteredSpenders.length;
  useEffect(() => { setTgPage(0); }, [tgFilteredLen]);
  ...
  const handleQuickAdd = async () => {
    if(!isValidHandle(quickForm.spender)) { addToast(fr?"Nom du spender requis":"Spender name required","error"); return; }
    if(!isValidAmount(quickForm.amount)) { addToast(fr?"Montant invalide (doit être > 0)":"Invalid amount (must be > 0)","error"); return; }
    setSaving(true);
    const isUUID = (v) => typeof v === "string" && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(v);
    try {
      const now = new Date().toISOString();
      const handle = sanitizeHandle(quickForm.spender);
      try {
        await safeUpsertSpender({handle, name: handle, status: "active", last_seen: now});
    ...
    <div>
          ...
          <div className="card" style={{marginBottom: 16}}>
            ...
            {importLog.length > 0 && (
              <div style={{background: "rgba(0,0,0,0.2)", borderRadius: 8, padding: 12, maxHeight: 300, overflow: "auto"}}>
                {importLog.map((log, i) => (
                  <div key={"log-" + i} style={{fontSize: 12, padding: "3px 0", color: log.type === "error" ? "var(--danger)" : log.type === "success" ? "var(--success)" : "var(--text2)"}}>
      ...
      {activeView === "fiches" && (
        <div>
          ...
          {/* Sort + Pagination */}
          {(()=>{
            const sortedTgSp = tgSort.doSort(filteredSpenders, {
              handle: x=>(x.handle||"").toLowerCase(),
              ltv: x=>x.ltv||0,
              nbTx: x=>x.nbTx||0,
              avgTicket: x=>x.avgTicket||0,
              modelName: x=>(x.modelName||"").toLowerCase(),
              recencyDays: x=>x.recencyDays||999,
            });
            const tgTotalPages = Math.max(1, Math.ceil(sortedTgSp.length / ROWS_PER_PAGE));
            const safeTgPage = Math.min(tgPage, tgTotalPages - 1);
            const pagedTgSp = sortedTgSp.slice(safeTgPage * ROWS_PER_PAGE, (safeTgPage+1) * ROWS_PER_PAGE);
            return <>
          <div style={{display:"flex",gap:4,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            <span style={{fontSize:11,color:"var(--text-tertiary)",marginRight:8}}>{fr?"Trier par":"Sort by"}:</span>
            {[{k:"ltv",l:"LTV"},{k:"nbTx",l:"TX"},{k:"avgTicket",l:fr?"Moy":"Avg"},{k:"handle",l:"Nom"},{k:"modelName",l:fr?"Modèle":"Model"},{k:"recencyDays",l:fr?"Récence":"Recency"}].map(s=>(
              <button key={s.k} className={`filter-chip ${tgSort.sort.key===s.k?"active":""}`} onClick={()=>tgSort.onSort(s.k)} style={{fontSize:10,padding:"3px 10px"}}>
                {s.l}{tgSort.sort.key===s.k?(tgSort.sort.dir==="asc"?" ↑":" ↓"):""}
              </button>
            ))}
          </div>
          {/* Cards grid */}
          ...
          <PaginationBar page={safeTgPage} totalPages={tgTotalPages} totalItems={sortedTgSp.length} lang={lang} onPageChange={setTgPage}/>
          </>;
          })()}
        </div>
      )}
              ...
              {s.contenu && s.contenu.length > 0 && (
                <div style={{marginBottom: 16}}>
                  ...
                  <div style={{display: "flex", gap: 4, flexWrap: "wrap"}}>
                    {s.contenu.map((c, i) => (
                      <span key={c + "-" + i} style={{background: "var(--accent-subtle)", color: "var(--accent)", padding: "2px 8px", borderRadius: 8, fontSize: 11}}>{c}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {s.txList.length === 0 ? (
                ...
                <div style={{overflowX: "auto"}}>
                  <table style={{width: "100%", fontSize: 12, borderCollapse: "collapse"}}>
                    ...
                    <tbody>
                      {s.txList.sort((a, b) => (b.txDate || "").localeCompare(a.txDate || "")).map((tx, i) => (
                        <tr key={tx.txId || ("tx-" + i)} style={{borderBottom: "1px solid var(--border-subtle)"}}>
...
module.exports.default = TelegramTab;
