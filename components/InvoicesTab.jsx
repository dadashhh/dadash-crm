const InvoicesTab = React.memo(({ user, lang, invoices, txs, profiles, providerPayouts, onRefresh }) => {
  ...
  const invSort = useSortable("created_at","desc");
  ...
  // KPIs
  const { totalFacture, countThisMonth, outgoing, outgoingTotal, incoming, incomingTotal, pendingTotal } = useMemo(() => {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const out = filteredByDate.filter(i => i.type === "outgoing_chatter" || i.type === "outgoing_model");
    const inc = filteredByDate.filter(i => i.type === "incoming_provider");
    const pend = filteredByDate.filter(i => i.status === "sent" || i.status === "draft" || i.status === "overdue");
    return {
      totalFacture: filteredByDate.reduce((s, i) => s + Number(i.total || 0), 0),
      countThisMonth: allInvoices.filter(i => new Date(i.created_at) >= monthStart).length,
      outgoing: out, outgoingTotal: out.reduce((s, i) => s + Number(i.total || 0), 0),
      incoming: inc, incomingTotal: inc.reduce((s, i) => s + Number(i.total || 0), 0),
      pendingTotal: pend.reduce((s, i) => s + Number(i.total || 0), 0),
    };
  }, [filteredByDate, allInvoices]);
  ...
  return React.createElement("div", null,
    ...
    showCreate && React.createElement("div", { className: "card", style: { marginBottom: 20 } },
      ...
      React.createElement("div", { style: { display: "flex", gap: 12, marginTop: 16 } },
        React.createElement("button", { className: "btn btn-success", onClick: handleCreate, disabled: saving }, saving ? React.createElement(React.Fragment, null, React.createElement(BtnSpinner), " ") : (fr ? "Créer facture" : "Create invoice")),
    ...
    // Table
    (()=>{
      if (filtered.length === 0) return React.createElement("div", { className: "card", style: { textAlign: "center", padding: 40, color: "var(--text-tertiary)" } }, fr ? "Aucune facture trouvée" : "No invoices found");
      const sortedInv = invSort.doSort(filtered, {
        invoice_number: x=>(x.invoice_number||"").toLowerCase(),
        created_at: x=>new Date(x.created_at||0).getTime(),
        type: x=>(x.type||""),
        from_to: x=>((x.from_name||"")+" "+(x.to_name||"")).toLowerCase(),
        total: x=>Number(x.total)||0,
        status: x=>(x.status||""),
      });
      const invPages = Math.max(1, Math.ceil(sortedInv.length / ROWS_PER_PAGE));
      const safeInvPage = Math.min(invPage, invPages - 1);
      const pagedInv = sortedInv.slice(safeInvPage * ROWS_PER_PAGE, (safeInvPage+1) * ROWS_PER_PAGE);
      return React.createElement("div", null,
        React.createElement("div", { className: "table-wrap" },
          React.createElement("table", { className: "table", style: { fontSize: 12 } },
            React.createElement("thead", null,
              React.createElement("tr", null,
                React.createElement(SortTh, { label: "N\u00B0 Facture", sortKey: "invoice_number", sort: invSort.sort, onSort: invSort.onSort }),
                React.createElement(SortTh, { label: "Date", sortKey: "created_at", sort: invSort.sort, onSort: invSort.onSort }),
                React.createElement(SortTh, { label: "Type", sortKey: "type", sort: invSort.sort, onSort: invSort.onSort }),
                React.createElement(SortTh, { label: fr ? "De \u2192 \u00C0" : "From \u2192 To", sortKey: "from_to", sort: invSort.sort, onSort: invSort.onSort }),
                React.createElement(SortTh, { label: fr ? "Montant" : "Amount", sortKey: "total", sort: invSort.sort, onSort: invSort.onSort }),
                React.createElement(SortTh, { label: "Status", sortKey: "status", sort: invSort.sort, onSort: invSort.onSort }),
                ...
                return React.createElement("tr", { key: inv.id },
                  ...
                  React.createElement("td", null,
                    React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap" } },
                      React.createElement("button", { className: "btn", style: { fontSize: 10, padding: "3px 8px" }, onClick: () => downloadInvoicePDF(inv), title: "PDF" }, "\u{1F4C4}"),
                      React.createElement("button", { className: "btn", style: { fontSize: 10, padding: "3px 8px" }, onClick: () => { setShowEmail(inv); setEmailTo(inv.to_details?.email || inv.sent_to_email || ""); }, title: "Email" }, "\u{1F4E7}"),
                      (inv.status === "draft" || inv.status === "sent") && React.createElement("button", { className: "btn", style: { fontSize: 10, padding: "3px 8px", color: "var(--danger)" }, onClick: () => handleCancel(inv), title: fr ? "Annuler" : "Cancel" }, "\u274C"),
                    )
                  ),
                );
              })
        ...
        React.createElement(PaginationBar, { page: safeInvPage, totalPages: invPages, totalItems: sortedInv.length, lang: lang, onPageChange: setInvPage }),
      );
    })(),
  );
});

module.exports.default = InvoicesTab;
